# TYPE OF CATALOG

CATALOG_CART = "cc"
CATALOG_GENERAL = "cg"

CATALOG_TYPE_CHOICE = (
    (CATALOG_CART, "Carta (Actividad Gastronómica)"),
    (CATALOG_GENERAL, "Catalógo General"),
)

# BUSINESS TYPE OF E-COMMERCE

BUSINESS_TO_BUSINESS = "b2b"
BUSINESS_TO_CONSUMER = "b2c"
CONSUMER_TO_CONSUMER = "c2c"
CONSUMER_TO_BUSINESS = "c2b"
BUSINESS_TO_ADMINISTRATION = "b2a"
CONSUMER_TO_ADMINISTRATION = "c2a"

BUSINESS_ECOMMERCE_TYPE_CHOICES = (
    (BUSINESS_TO_BUSINESS, "Venta Mayorista"),
    (BUSINESS_TO_CONSUMER, "Venta Minorista"),
    (CONSUMER_TO_CONSUMER, "Comércio entre Consumidores"),
    (CONSUMER_TO_BUSINESS, "Comércio Especializado"),
    (BUSINESS_TO_ADMINISTRATION, "Venta a la Administración Pública"),
    (CONSUMER_TO_ADMINISTRATION, "Servicios Especializados a la Administración Pública"),
)


# BUSINESS COUNTRY

ARGENTINA = 'Argentina'
ALBANIA = 'Albania'
ALEMANIA = 'Alemania'
ANDORRA = 'Andorra'
ANGUILA = 'Anguila'
ARABIA_SAUDITA = 'Arabia Saudita'
ARMENIA = 'Armenia'
ARUBA = 'Aruba'
AUSTRALIA = 'Australia'
AUSTRIA = 'Austria'
AZERBAIYAN = 'Azerbaiyán'
BAHAMAS = 'Bahamas'
BARBADOS = 'Barbados'
BELARUS = 'Belarús'
BELICE = 'Belice'
BERMUDAS = 'Bermudas'
BOLIVIA = 'Bolivia'
BRASIL = 'Brasil'
BULGARIA = 'Bulgaria'
BURKINA_FASO = 'Burkina Faso'
BELGICA = 'Bélgica'
CAIMAN = 'Caimán'
CAMERUN = 'Camerún'
CANADA = 'Canadá'
CHECA = 'Checa'
CHILE = 'Chile'
CHINA = 'China'
CHIPRE = 'Chipre'
CIUDAD_DEL_VATICANO = 'Ciudad del Vaticano'
COLOMBIA = 'Colombia'
COREA_DEL_SUR = 'Corea del Sur'
COSTA_RICA = 'Costa Rica'
CROACIA = 'Croacia'
CUBA = 'Cuba'
CURAZAO = 'Curazao'
DINAMARCA = 'Dinamarca'
ECUADOR = 'Ecuador'
EL_SALVADOR = 'El Salvador'
EMIRATOS_ARABES_UNIDOS = 'Emiratos Árabes Unidos'
ESLOVAQUIA = 'Eslovaquia'
ESLOVENIA = 'Eslovenia'
ESPANA = 'España'
ESTADOS_UNIDOS = 'Estados  Unidos'
ESTONIA = 'Estonia'
FEROE = 'Feroe'
FILIPINAS = 'Filipinas'
FINLANDIA = 'Finlandia'
FRANCIA = 'Francia'
GABON = 'Gabon'
GEORGIA = 'Georgia'
GHANA = 'Ghana'
GIBRALTAR = 'Gibraltar'
GRECIA = 'Grecia'
GROENLANDIA = 'Groenlandia'
GUATEMALA = 'Guatemala'
GUERNSEY = 'Guernsey'
HONDURAS = 'Honduras'
HONG_KONG = 'Hong Kong'
HUNGRIA = 'Hungría'
INDIA = 'India'
INDONESIA = 'Indonesia'
IRLANDA = 'Irlanda'
ISLA_DE_MAN = 'Isla de Man'
ISLANDIA = 'Islandia'
ISLAS_COOK = 'Islas Cook'
ISLAS_MARSHALL = 'Islas Marshall'
ISLAS_TURCAS_Y_CAICOS = 'Islas Turcas y Caicos'
ISLAS_VIRGENES_BRITANICAS = 'Islas Vírgenes Británicas'
ISRAEL = 'Israel'
ITALIA = 'Italia'
JAMAICA = 'Jamaica'
JAPON = 'Japón'
JERSEY = 'Jersey'
KAZAJISTAN = 'Kazajistán'
KENIA = 'Kenia'
KUWAIT = 'Kuwait'
LETONIA = 'Letonia'
LIECHTENSTEIN = 'Liechtenstein'
LITUANIA = 'Lituania'
LUXEMBURGO = 'Luxemburgo'
MACAO = 'Macao'
MACEDONIA = 'Macedonia'
MALASIA = 'Malasia'
MALTA = 'Malta'
MARRUECOS = 'Marruecos'
MAURICIO = 'Mauricio'
MOLDAVIA = 'Moldavia'
MONTSERRAT = 'Montserrat'
MEXICO = 'México'
MONACO = 'Mónaco'
NAURU = 'Nauru'
NIGERIA = 'Nigeria'
NIUE = 'Niue'
NORUEGA = 'Noruega'
NUEVA_ZELANDA = 'Nueva Zelanda'
PAKISTAN = 'Pakistán'
PANAMA = 'Panamá'
PARAGUAY = 'Paraguay'
PAISES_BAJOS = 'Países Bajos'
PERU = 'Perú'
POLONIA = 'Polonia'
PORTUGAL = 'Portugal'
REINO_UNIDO = 'Reino Unido'
REPUBLICA_DOMINICANA = 'República Dominicana'
RUMANIA = 'Rumania'
RUSIA = 'Rusia'
SAINT_MARTEEN = 'Saint Marteen'
SAMOA = 'Samoa'
SAN_CRISTOBAL_Y_NIEVES = 'San Cristóbal y Nieves'
SAN_MARINO = 'San Marino'
SAN_VICENTE_Y_GRANADINAS = 'San Vicente y Granadinas'
SANTA_LUCIA = 'Santa Lucia'
SEICHELES = 'Seicheles'
SENEGAL = 'Senegal'
SERBIA = 'Serbia'
SINGAPUR = 'Singapur'
SUDAFRICA = 'Sudáfrica'
SUECIA = 'Suecia'
SUIZA = 'Suiza'
TURKMENISTAN = 'Turkmenistán'
TURQUIA = 'Turquía'
TUNEZ = 'Túnez'
UCRANIA = 'Ucrania'
UGANDA = 'Uganda'
URUGUAY = 'Uruguay'
VENEZUELA = 'Venezuela'

COUNTRY_CHOICES = (
    (ARGENTINA , 'Argentina'),
    (ALBANIA , 'Albania'),
    (ALEMANIA , 'Alemania'),
    (ANDORRA , 'Andorra'),
    (ANGUILA , 'Anguila'),
    (ARABIA_SAUDITA , 'Arabia Saudita'),
    (ARMENIA , 'Armenia'),
    (ARUBA , 'Aruba'),
    (AUSTRALIA , 'Australia'),
    (AUSTRIA , 'Austria'),
    (AZERBAIYAN , 'Azerbaiyán'),
    (BAHAMAS , 'Bahamas'),
    (BARBADOS , 'Barbados'),
    (BELARUS , 'Belarús'),
    (BELICE , 'Belice'),
    (BERMUDAS , 'Bermudas'),
    (BOLIVIA , 'Bolivia'),
    (BRASIL , 'Brasil'),
    (BULGARIA , 'Bulgaria'),
    (BURKINA_FASO , 'Burkina Faso'),
    (BELGICA , 'Bélgica'),
    (CAIMAN , 'Caimán'),
    (CAMERUN , 'Camerún'),
    (CANADA , 'Canadá'),
    (CHECA , 'Checa'),
    (CHILE , 'Chile'),
    (CHINA , 'China'),
    (CHIPRE , 'Chipre'),
    (CIUDAD_DEL_VATICANO , 'Ciudad del Vaticano'),
    (COLOMBIA , 'Colombia'),
    (COREA_DEL_SUR , 'Corea del Sur'),
    (COSTA_RICA , 'Costa Rica'),
    (CROACIA , 'Croacia'),
    (CUBA , 'Cuba'),
    (CURAZAO , 'Curazao'),
    (DINAMARCA , 'Dinamarca'),
    (ECUADOR , 'Ecuador'),
    (EL_SALVADOR , 'El Salvador'),
    (EMIRATOS_ARABES_UNIDOS , 'Emiratos Árabes Unidos'),
    (ESLOVAQUIA , 'Eslovaquia'),
    (ESLOVENIA , 'Eslovenia'),
    (ESPANA , 'España'),
    (ESTADOS_UNIDOS , 'Estados  Unidos'),
    (ESTONIA , 'Estonia'),
    (FEROE , 'Feroe'),
    (FILIPINAS , 'Filipinas'),
    (FINLANDIA , 'Finlandia'),
    (FRANCIA , 'Francia'),
    (GABON , 'Gabon'),
    (GEORGIA , 'Georgia'),
    (GHANA , 'Ghana'),
    (GIBRALTAR , 'Gibraltar'),
    (GRECIA , 'Grecia'),
    (GROENLANDIA , 'Groenlandia'),
    (GUATEMALA , 'Guatemala'),
    (GUERNSEY , 'Guernsey'),
    (HONDURAS , 'Honduras'),
    (HONG_KONG , 'Hong Kong'),
    (HUNGRIA , 'Hungría'),
    (INDIA , 'India'),
    (INDONESIA , 'Indonesia'),
    (IRLANDA , 'Irlanda'),
    (ISLA_DE_MAN , 'Isla de Man'),
    (ISLANDIA , 'Islandia'),
    (ISLAS_COOK , 'Islas Cook'),
    (ISLAS_MARSHALL , 'Islas Marshall'),
    (ISLAS_TURCAS_Y_CAICOS , 'Islas Turcas y Caicos'),
    (ISLAS_VIRGENES_BRITANICAS , 'Islas Vírgenes Británicas'),
    (ISRAEL , 'Israel'),
    (ITALIA , 'Italia'),
    (JAMAICA , 'Jamaica'),
    (JAPON , 'Japón'),
    (JERSEY , 'Jersey'),
    (KAZAJISTAN , 'Kazajistán'),
    (KENIA , 'Kenia'),
    (KUWAIT , 'Kuwait'),
    (LETONIA , 'Letonia'),
    (LIECHTENSTEIN , 'Liechtenstein'),
    (LITUANIA , 'Lituania'),
    (LUXEMBURGO , 'Luxemburgo'),
    (MACAO , 'Macao'),
    (MACEDONIA , 'Macedonia'),
    (MALASIA , 'Malasia'),
    (MALTA , 'Malta'),
    (MARRUECOS , 'Marruecos'),
    (MAURICIO , 'Mauricio'),
    (MOLDAVIA , 'Moldavia'),
    (MONTSERRAT , 'Montserrat'),
    (MEXICO , 'México'),
    (MONACO , 'Mónaco'),
    (NAURU , 'Nauru'),
    (NIGERIA , 'Nigeria'),
    (NIUE , 'Niue'),
    (NORUEGA , 'Noruega'),
    (NUEVA_ZELANDA , 'Nueva Zelanda'),
    (PAKISTAN , 'Pakistán'),
    (PANAMA , 'Panamá'),
    (PARAGUAY , 'Paraguay'),
    (PAISES_BAJOS , 'Países Bajos'),
    (PERU , 'Perú'),
    (POLONIA , 'Polonia'),
    (PORTUGAL , 'Portugal'),
    (REINO_UNIDO , 'Reino Unido'),
    (REPUBLICA_DOMINICANA , 'República Dominicana'),
    (RUMANIA , 'Rumania'),
    (RUSIA , 'Rusia'),
    (SAINT_MARTEEN , 'Saint Marteen'),
    (SAMOA , 'Samoa'),
    (SAN_CRISTOBAL_Y_NIEVES , 'San Cristóbal y Nieves'),
    (SAN_MARINO , 'San Marino'),
    (SAN_VICENTE_Y_GRANADINAS , 'San Vicente y Granadinas'),
    (SANTA_LUCIA , 'Santa Lucia'),
    (SEICHELES , 'Seicheles'),
    (SENEGAL , 'Senegal'),
    (SERBIA , 'Serbia'),
    (SINGAPUR , 'Singapur'),
    (SUDAFRICA , 'Sudáfrica'),
    (SUECIA , 'Suecia'),
    (SUIZA , 'Suiza'),
    (TURKMENISTAN , 'Turkmenistán'),
    (TURQUIA , 'Turquía'),
    (TUNEZ , 'Túnez'),
    (UCRANIA , 'Ucrania'),
    (UGANDA , 'Uganda'),
    (URUGUAY , 'Uruguay'),
    (VENEZUELA , 'Venezuela'),
)
